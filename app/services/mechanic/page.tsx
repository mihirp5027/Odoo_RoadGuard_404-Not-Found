  ...
  const getUserLocation = async () => {
    ...
    try {
      ...
      if (navigator.geolocation) {
        ...
        // Get address from coordinates using OpenCage reverse geocoding
        ...
        if (data.results && data.results.length > 0) {
          ...
          toast({
            ...
            description: "Successfully found your precise location and nearby mechanics.",
          });
          return;
        }
      }
      ...
      const response = await fetch(`https://api.opencagedata.com/geocode/v1/json?q=ip&key=${OPENCAGE_API_KEY}`);
      const data = await response.json();
      ...
      if (data.results && data.results.length > 0) {
        ...
        toast({
          ...
          description: "Found your approximate location based on IP address. For more accurate results, please enable location services.",
          variant: "default",
        });
      } else {
      ...
      toast({
        ...
        description: "Could not get your location. Please enable location services for better accuracy.",
        variant: "destructive",
      });
    } finally {
  ...
  const handleSubmitRequest = async () => {
    ...
    try {
      ...
      if (isManualVehicleEntry) {
        ...
        // The response might be in vehicles array or directly in the vehicle object
        vehicleId = vehicleData.vehicles ? 
          vehicleData.vehicles[vehicleData.vehicles.length - 1]._id : 
          vehicleData.vehicle?._id;

...
// Assign worker to a service request
export const assignWorkerToRequest = async (req: Request, res: Response) => {
  try {
    ...
    // Find the worker
    const worker = await Worker.findOne({ _id: workerId, mechanicId: mechanic._id });
    if (!worker) {
    ...
    if (!request) {
      return res.status(404).json({ error: 'Service request not found or not in Accepted status' });
    }
...
export const getAvailableWorkers = async (req: Request, res: Response) => {
  try {
    ...
    // Find the mechanic to get their ID
    const mechanic = await Mechanic.findOne({ userId });
    if (!mechanic) {
      return res.status(404).json({ error: 'Mechanic not found' });
    }

    // Find all available workers for this mechanic
    const workers = await Worker.find({
      mechanicId: mechanic._id,
      currentStatus: 'available',
      isActive: true
    }).select('name mobileNumber currentStatus');
